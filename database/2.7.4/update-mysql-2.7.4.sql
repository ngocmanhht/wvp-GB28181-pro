...
/*
* 20260417 willwvp_device_mobile_positionChanged from an exclusive national standard location record table to a location record table shared by general channels
*/
...
BEGIN
 IF NOT EXISTS (SELECT column_name FROM information_schema.columns
    ...
    THEN
    ALTER TABLE wvp_device_mobile_position ADD timestamp BIGINT COMMENT 'Reporting time';
END IF;
IF EXISTS (SELECT column_name FROM information_schema.columns
...
/*
* 20260521 addwvp_device_channelUnique constraint to prevent repeated writing of channels
*/
...
BEGIN
    ...
    THEN
        -- use GROUP BY + LEFT JOIN Alternative self-join DELETE
        DELETE t1
