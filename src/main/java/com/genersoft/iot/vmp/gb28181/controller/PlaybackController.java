...
public class PlaybackController {
	...
	@Operation(summary = "Start video playback", security = @SecurityRequirement(name = JwtUtils.HEADER))
	...
	public DeferredResult<WVPResult<StreamContent>> start(HttpServletRequest request, @PathVariable String deviceId, @PathVariable String channelId,
		...
		log.info("[Video playback] deviceId: {}, channelId: {}, startTime: {}, endTime: {}", deviceId, channelId, startTime, endTime);
		...
		if (device == null) {
			log.warn("[Video playback] Device not found deviceId: {},channelId:{}", deviceId, channelId);
		...
		if (channel == null) {
			log.warn("[Video playback] Channel not found deviceId: {},channelId:{}", deviceId, channelId);
	...
	@Operation(summary = "Playback paused", security = @SecurityRequirement(name = JwtUtils.HEADER))
	@Parameter(name = "streamId", description = "replay streamID", required = true)
	@GetMapping("/pause/{streamId}")
	public void playbackPause(@PathVariable String streamId) {
		log.info("[Playback paused] streamId: {}", streamId);
		try {
	...
	@Operation(summary = "Playback resume", security = @SecurityRequirement(name = JwtUtils.HEADER))
	@Parameter(name = "streamId", description = "replay streamID", required = true)
	...
	@Operation(summary = "Playback drag and play", security = @SecurityRequirement(name = JwtUtils.HEADER))
	@Parameter(name = "streamId", description = "replay streamID", required = true)
	@Parameter(name = "seekTime", description = "Drag offset, units", required = true)
	...
	@Operation(summary = "Playback at double speed", security = @SecurityRequirement(name = JwtUtils.HEADER))
	@Parameter(name = "streamId", description = "replay streamID", required = true)
	@Parameter(name = "speed", description = "Double speed0.25 0.5 1、2、4、8", required = true)
	@GetMapping("/speed/{streamId}/{speed}")
	public void playSpeed(@PathVariable String streamId, @PathVariable Double speed) {
		Assert.notNull(speed, "Double speed does not exist");
