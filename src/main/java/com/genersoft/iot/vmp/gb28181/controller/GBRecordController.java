...
@Tag(name  = "National standard video")
@Slf4j
@RestController
...
public class GBRecordController {
	...
	public DeferredResult<WVPResult<RecordInfo>> recordinfo(@PathVariable String deviceId, @PathVariable String channelId, String startTime, String endTime){
		...
		if (device == null) {
			throw new ControllerException(ErrorCode.ERROR100.getCode(), deviceId + " does not exist");
		}
		DeviceChannel channel = channelService.getOneForSource(device.getId(), channelId);
		if (channel == null) {
			throw new ControllerException(ErrorCode.ERROR100.getCode(), channelId + " does not exist");
		}
	...
	@Operation(summary = "Start historical media download", security = @SecurityRequirement(name = JwtUtils.HEADER))
	@Parameter(name = "deviceId", description = "Equipment national standard number", required = true)
	@Parameter(name = "channelId", description = "Channel national standard number", required = true)
	@Parameter(name = "startTime", description = "start time", required = true)
	@Parameter(name = "endTime", description = "end time", required = true)
	@Parameter(name = "downloadSpeed", description = "Download twice as fast", required = true)
	...
	public DeferredResult<WVPResult<StreamContent>> download(HttpServletRequest request, @PathVariable String deviceId, @PathVariable String channelId,
		...
		log.info("[Start historical media download] deviceId: {}, channelId: {}, startTime: {}, endTime: {}, downloadSpeed: {}", deviceId, channelId, startTime, endTime, downloadSpeed);
		...
		Device device = deviceService.getDeviceByDeviceId(deviceId);
		if (device == null) {
			log.warn("[Start historical media download] Device not found deviceId: {},channelId:{}", deviceId, channelId);
		...
		if (channel == null) {
			log.warn("[Start historical media download] Channel not found deviceId: {},channelId:{}", deviceId, channelId);
			throw new ControllerException(ErrorCode.ERROR100.getCode(), "Channel not found：" + channelId);
		}
	...
	@Operation(summary = "Stop historical media downloads", security = @SecurityRequirement(name = JwtUtils.HEADER))
	@Parameter(name = "deviceId", description = "Equipment national standard number", required = true)
	@Parameter(name = "channelId", description = "Channel national standard number", required = true)
	...
	public void downloadStop(@PathVariable String deviceId, @PathVariable String channelId, @PathVariable String stream) {
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format("Device history media download stopped API call，deviceId/channelId：%s_%s", deviceId, channelId));
		}
		...
		Device device = deviceService.getDeviceByDeviceId(deviceId);
		if (device == null) {
			throw new ControllerException(ErrorCode.ERROR400.getCode(), "Equipment：" + deviceId + " not found");
		}
		DeviceChannel deviceChannel = channelService.getOneForSource(deviceId, channelId);
		if (deviceChannel == null) {
			throw new ControllerException(ErrorCode.ERROR400.getCode(), "channel：" + channelId + " not found");
		}
		playService.stop(InviteSessionType.DOWNLOAD, device, deviceChannel, stream);
	}

	@Operation(summary = "Get historical media download progress", security = @SecurityRequirement(name = JwtUtils.HEADER))
	@Parameter(name = "deviceId", description = "Equipment national standard number", required = true)
	@Parameter(name = "channelId", description = "Channel national standard number", required = true)
	@Parameter(name = "stream", description = "flowID", required = true)
	...
	public StreamContent getProgress(@PathVariable String deviceId, @PathVariable String channelId, @PathVariable String stream) {
		Device device = deviceService.getDeviceByDeviceId(deviceId);
		if (device == null) {
			log.warn("[Get historical media download progress] Device not found deviceId: {},channelId:{}", deviceId, channelId);
			throw new ControllerException(ErrorCode.ERROR100.getCode(), "Device not found：" + deviceId);
		}

		DeviceChannel channel = channelService.getOne(deviceId, channelId);
		if (channel == null) {
			log.warn("[Get historical media download progress] Channel not found deviceId: {},channelId:{}", deviceId, channelId);
			throw new ControllerException(ErrorCode.ERROR100.getCode(), "Channel not found：" + channelId);
		}
